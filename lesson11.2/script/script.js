...
// функция start, которая запускает все остальные
AppData.prototype.start = function () {
    ...
    // ДЕЛАЕТ ВСЕ INPUT'ы НЕАКТИВНЫМИ!
...
AppData.prototype.reset = function () {
    ...
    // ДЕЛАЕТ ВСЕ INPUT'ы АКТИВНЫМИ!
    start.style.display = '';
    cancel.style.display = '';
    salaryAmount.removeAttribute('disabled', '');

    incomeItem.forEach(function (item) {
        item.querySelector('.income-title').removeAttribute('disabled', 'disabled');
        item.querySelector('.income-amount').removeAttribute('disabled', 'disabled');
    });
    ...
    expensesItems.forEach(function (item) {
        item.querySelector('.expenses-title').removeAttribute('disabled', 'disabled');
        item.querySelector('.expenses-amount').removeAttribute('disabled', 'disabled');
    });

    additionalExpensesItem.removeAttribute('disabled', 'disabled');
    depositCheck.removeAttribute('disabled', 'disabled');
    targetAmount.removeAttribute('disabled', 'disabled');

};

//МЕТОД ПОКАЗЫВАЕТ ВСЕ РЕЗУЛЬТАТЫ
...
// ДОБАВЛЯЕТ ОБЯЗАТЕЛЬНЫЕ РАСХОДЫ
...
//     expensesItems = document.querySelectorAll('.expenses-items');
//     incomeItem = document.querySelectorAll('.income-items');
//     if(expensesItems.length === 3 && incomeItem.length === 3){
//         expensesButton.style.display = 'none';
//         incomeButton.style.display = 'none';
//     }
// };
...
let bindReset = appData.reset.bind(appData); // BIND ДЛЯ ОЧИСТКИ ФОРМЫ
appData.eventsListeners(); // ОБРАБОТЧИКИ СОБЫТИЙ